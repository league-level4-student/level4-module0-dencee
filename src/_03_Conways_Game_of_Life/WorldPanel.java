...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
    ...
    public void step() {
        ...
        for( int y = 0; y < livingNeighbors.length; y++ ) {
            for( int x = 0; x < livingNeighbors[0].length; x++ ) {

                livingNeighbors[y][x] = getLivingNeighbors( y, x );
    ...
    public int getLivingNeighbors(int x, int y) {
        ...
        if( x + 1 < cells.length && y - 1 >= 0 ) {
            if( cells[ x+1 ][ y-1 ].isAlive ) {
                neighbors++;
            }
        }
        
        if( x - 1 >= 0 && y + 1 < cells.length ) {
            if( cells[ x-1 ][ y+1 ].isAlive ) {
                neighbors++;
            }
        }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        int x = e.getX();
        int y = e.getY();
        cells[y / this.cellSize][x / this.cellSize].isAlive = true;
